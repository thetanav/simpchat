          ...
          <div key={key} className="space-y-2 p-3 border rounded-md bg-muted/30">
            ...
            {output.stdout && (
              <div>
                ...
                <CodeBlock code={output.stdout} language="text" />
              </div>
            )}
            {output.stderr && (
              <div>
                <p className="text-xs font-semibold text-red-500">Stderr:</p>
                <CodeBlock code={output.stderr} language="text" />
              </div>
            )}
            {output.error && (
...
function SearchResultsDialog({ results }: { results: SearchResult[] }) {
...
export default function ChatClientPage({
  ...
  const extractSearchResults = useCallback(
    (message: UIMessage): SearchResult[] => {
